...
static void here_doc_read_line(t_ast *ast, t_red_prop *prop, int *pipe_fds)
{
    ...
    while (1)
    {
        ...
        if ((ft_strncmp(prop->file_name, line, ft_strlen(prop->file_name)) == 0) && line[ft_strlen(prop->file_name)] == '\n')
        {
            free(line);
            break ;
        }
...
//
// Note:
// - A pipe initially has exactly one read (fds[0]) and one write (fds[1]) endpoint.
...
// - Once reading is complete, the read end must also be closed to free resources.
...
static void open_file_helper(t_ast *ast, t_red_prop *prop, bool is_in)
{
    ...
    if (access(prop->file_name, F_OK) == 0 && access(prop->file_name, access_code) == -1)
